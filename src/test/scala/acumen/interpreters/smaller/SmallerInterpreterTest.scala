...
class SmallerInterpreterTest extends InterpreterTestBase {
  override def interpreter : CStoreInterpreter = SmallerInterpreter

  override def suiteName = "New SmallerInterpreterTest"

  override def run(in: InputStreamReader) = {    
    val ast = Parser.run(Parser.prog, in)
    val des = Desugarer().run(ast)
    for (_ <- (SmallerInterpreter.run(des).ctrace)) ()
  }

  test("StoreConversions1") {
    import SmallerInterpreter._
    val txt = """
...
"""
  ...
  // testExamples takes a function that returns true if model should be
  // skipped for whatever reason
  // the ping pong models are ignored since this interpreter currently
  // is using the old semantics
