...
object SymbolicCommon {
  ...
  def inverse(matrix: MatrixE): MatrixE = {
    if (matrix.length == 2 && matrix(0).length == 2) {
      ...
      val det = mkOp("/", rationalone, mkOp("-", mkOp("*", a, d), mkOp("*", b, c)))
      Array(Array(mkOp("*", det, d), mkOp("*", det, mkOp("*", rationalmone, b))),
        Array(mkOp("*", det, rationalmone), mkOp("*", det, a)))
    } else
      matrixScaleOp("*", transpose(cofactor(matrix)), mkOp("/", rationalone, determinant(matrix)))
  }
  ...
  def determinant(matrix: MatrixE): Expr = {
    ...
    var sum = rationalzero.asInstanceOf[Expr]
    var s: Double = 1
