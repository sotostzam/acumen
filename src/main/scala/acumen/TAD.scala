...
object TAD extends App {
  ...
  abstract class TDifAsIntegral[V: Integral] extends DifAsIntegral[V,TDif[V]] with Integral[TDif[V]] {
    override def dif(v: SafeSeq[V]): TDif[V] = TDif(v)
    /* Caches */
...
//      }))
    ...
    def firstNonZero(x: TDif[V]): Option[Int] = x.coeff.indexWhere(_ != zeroOfV)
  }
...
//        val coeff = (l(k0) / r(k0)) #:: Stream.empty[V]
////        for (k <- 1 to n - k0 - 1)
...
//      })
  ...
  case class TDif[V](coeff: SafeSeq[V]) extends Dif[V]
  object TDif {
//    case class ConstantSeq[V: Integral](v: V) extends Seq[V] {
//      private val zero = implicitly[Integral[V]].zero
//      def apply(i: Int) = if (i == 0) v else zero
//      def iterator: Iterator[V] = new Iterator[V]{
//        private var i = 0
//        def hasNext = true
//        def next() = { val n = apply(i); i += 1; n }
//      }
//      def length: Int = -1
//    }
    /** Lift a constant value of type A to a TDif. */
//    def constant[V: Integral](a: V) = TDif(a #:: Stream.continually(implicitly[Integral[V]].zero))
    def constant[V: Integral](a: V) = TDif(SafeStream(Map(0 -> a), Right(implicitly[Integral[V]].zero)))
  }
