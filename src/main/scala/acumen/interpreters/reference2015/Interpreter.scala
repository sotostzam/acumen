...
object Interpreter extends acumen.CStoreInterpreter {
  ...
  def init(prog:Prog) : (Prog, SuperStore, SuperMetadata) = {
    ...
    val splitStore = splitIntervalsStore(fromCStore(st4))
    val mds = splitStore mapValues (testHypotheses(hyps, NoMetadata, _, 0)(NoBindings))
    (mprog, splitStore, mds)
  }
        ...
        val res = resultType match {
          case Initial | Discrete | Continuous => // Do discrete step or conclude discrete fixpoint
            ...
            val nonIdentityAs = (dasValues ++ nonClashingEqsValues).filterNot{ case (id, n, v) => 
              specialField(n.x) || v == getObjectField(id, n, st1) }
            /* Apply discrete assignments to _3D and _3DView variables */
            val threeDValues = dasValues.filter{ case (_, n, _) => specialField(n.x) }
