...
object Desugarer {
  ...
  def desugar(p:Prog, fs:List[Name], env:List[Name], is:List[Init]) 
    : (List[Name], List[Init]) = {
      is match {
        case Nil => (List(), Nil)
        case Init(x, rhs)::is1 =>
  ...
  def desugar(p:Prog, fs:List[Name], env:List[Name], e:ContinuousAction) 
        ...
        case Equation(lhs, rhs) => 
          ...
          dlhs match {
            case Dot(o,Name(f,n)) => 
              EquationT(dlhs,drhs) :: (
                for (k <- n until (0,-1))
                yield EquationI(Dot(o,Name(f, k-1)), Dot(o, Name(f,k)))
              ).toList
  ...
  def desugar(p:Prog, fs:List[Name], env:List[Name], e:DiscreteAction) 
    : List[DiscreteAction] = {
      val des = desugar(p, fs,env,_:Expr)
      e match {
